...
def get_questions(topic: str, db: Session = Depends(database.get_db)):
    try:
        ...
        for q in ai_questions:
            ...
            # Save to database to build up a robust fallback pool over time
        ...
        print(f"Error generating fresh questions, using fallback: {e}")
        db.rollback()
        
        # Fallback to the database so it never throws an error
        questions = (
            db.query(models.AIQuestion)
            .filter(models.AIQuestion.topic == topic)
