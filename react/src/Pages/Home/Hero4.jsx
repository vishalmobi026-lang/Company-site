      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-white text-gray-900 px-4 sm:px-6 py-16">
        ...
        <div className="max-w-7xl w-full text-center relative z-10">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8 -mt-8">
            {(() => {
                const activePricing = pricing.filter(p => !p.is_disabled);
                const featured = activePricing.filter(p => p.is_featured);
                const nonFeatured = activePricing.filter(p => !p.is_featured);
                let display = [];
